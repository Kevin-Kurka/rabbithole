  ...
  return (
    <>
      <sup
        ...
        className="text-crt-fg cursor-pointer hover:text-crt-accent font-bold"
      >
        [{index}]
      </sup>
      ...
      {showTooltip && (
        <div
          className="fixed bg-black border border-crt-border  shadow-lg p-3 z-50 max-w-xs"
          style={{ left: `${tooltipPos.x}px`, top: `${tooltipPos.y}px` }}
          onMouseEnter={() => setShowTooltip(true)}
          onMouseLeave={() => setShowTooltip(false)}
        >
          <div className="text-sm">
            <p className="font-semibold text-crt-fg mb-1">{props.title}</p>
            {props.publication && (
              <p className="text-crt-fg text-xs mb-1">
            ...
            <p className="text-crt-muted text-xs mb-2">
            ...
            {props.url && (
              <a
                ...
                className="text-crt-fg hover:text-crt-accent text-xs underline block"
              >
                Open source →
              </a>
            )}
