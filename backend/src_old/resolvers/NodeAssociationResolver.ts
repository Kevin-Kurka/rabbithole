    ...
    try {
      const result = await pool.query(
        `SELECT
           ...
           e.source_node_id,
           e.target_node_id,
           e.props->>'relationship' as relationship,
           (e.props->>'weight')::float as weight,
           e.created_at,
           n.id as target_id,
           n.props->>'title' as target_title,
           (n.props->>'weight')::float as target_weight,
      ...
      return result.rows.map((row) => ({
        ...
        confidence: row.weight || 0.5,
        relationshipType: row.relationship,
        createdAt: row.created_at,
        targetNode: {
          id: row.target_id,
          title: row.target_title,
          weight: row.target_weight || 0.5,
          props: typeof row.target_props === 'string' ? JSON.parse(row.target_props) : row.target_props,
